...
test('phone number is a valid phone number', () => {
...
test('phone number that does not start with "+" is not a valid value', () => {
    const phoneNumber = '516698556'
    expect(validateNumber(phoneNumber)).toBeFalsy()
})

test('phone number that ends with "+" is not a valid value', () => {
    const phoneNumber = '516698556+'
    expect(validateNumber(phoneNumber)).toBeFalsy()
})

test('phone number that does not have at least 4 digits is not a valid value', () => {
    const phoneNumber = '+516'
    expect(validateNumber(phoneNumber)).toBeFalsy()
})

test('phone number that has more than 16 digits is not a valid value', () => {
